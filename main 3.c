...
{ 
  ...
  a->veloX += (dt*a->accX)/10;
  a->veloY += (dt*a->accY)/10;
...

{
    ...
    for( int i=0; i < N; i++)
      {
	points[i].x = 200 * star[i].cordX + (X_SIZE / 2.0);
	points[i].y = 200 * star[i].cordY + (Y_SIZE / 2.0);
      }
}
